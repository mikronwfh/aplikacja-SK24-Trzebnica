...
import { Moon, Sun, LogOut, Shield, Menu, BookOpen, Music2, Star, MoreHorizontal, Settings } from "lucide-react";
    ...
    <div className="min-h-screen flex flex-col bg-background">
            ...
            {user ? (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="icon" aria-label="Konto"><Menu className="h-5 w-5" /></Button>
                </DropdownMenuTrigger>
      ...
      <nav className="md:hidden fixed bottom-0 inset-x-0 z-40 border-t bg-background/95 backdrop-blur">
        <ul className="grid grid-cols-4 text-xs">
          ...
          <TabItem to="/ulubione" icon={<Star className="h-5 w-5" />} label="Ulubione" />
    ...
    <li>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" side="top">
          ...
          {isInstructor && (
            <DropdownMenuItem asChild><Link to="/admin"><Settings className="mr-2 h-4 w-4" />Panel admina</Link></DropdownMenuItem>
          )}
          {hasUser ? (
