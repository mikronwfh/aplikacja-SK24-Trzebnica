...
const AdminMundur = () => {
  ...
  const load = async () => {
    const { data } = await supabase.from("mundur_entries").select("*").order("sort_order").order("title");
  ...
  const save = async () => {
    ...
    const { error } = editing.id
      ? await supabase.from("mundur_entries").update(payload).eq("id", editing.id)
      : await supabase.from("mundur_entries").insert(payload);
  ...
  const remove = async (id: string) => {
    ...
    const { error } = await supabase.from("mundur_entries").delete().eq("id", id);
    ...
    <div>
      <div className="flex justify-end mb-4">
        <Button onClick={() => { setEditing({ id: "", ...empty, sort_order: rows.length }); setOpen(true); }}>
          <Plus className="h-4 w-4 mr-2" />Dodaj element munduru
        </Button>
      </div>
      ...
      <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) setEditing(null); }}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{editing?.id ? "Edytuj element" : "Nowy element munduru"}</DialogTitle></DialogHeader>
          {editing && (
