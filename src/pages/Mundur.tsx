...
const Mundur = () => {
  const { user } = useAuth();
  const [entries, setEntries] = useState<MundurEntry[]>([]);
  const [q, setQ] = useState("");
  const [favs, setFavs] = useState<Set<string>>(new Set());

  useEffect(() => {
    supabase.from("mundur_entries").select("*").order("sort_order").order("title").then(({ data, error }) => {
      if (error) toast.error("Błąd ładowania munduru");
      else setEntries((data ?? []) as MundurEntry[]);
    });
  }, []);
  ...
  useEffect(() => {
    ...
    listFavorites(user.id).then((rows) => {
      setFavs(new Set(rows.filter((r) => r.item_type === "mundur").map((r) => r.item_id)));
    });
  }, [user]);
  ...
  const toggle = async (id: string) => {
    ...
    setFavs(next);
    try { await toggleFavorite(user.id, "mundur", id, on); }
    ...
    <div className="container py-6 max-w-3xl">
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h1 className="text-3xl font-bold">Mundur</h1>
        </div>
        <p className="text-sm text-muted-foreground">Opis munduru, zasady noszenia, ilustracje.</p>
      </div>
      <div className="relative mb-4">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
        <Input placeholder="Szukaj w mundurze…" value={q} onChange={(e) => setQ(e.target.value)} className="pl-9" />
      </div>

      {filtered.length === 0 && (
        <div className="text-center text-muted-foreground py-12">
          {entries.length === 0 ? "Brak wpisów. Instruktor doda opis munduru z panelu admina." : "Brak wyników."}
        </div>
      )}
